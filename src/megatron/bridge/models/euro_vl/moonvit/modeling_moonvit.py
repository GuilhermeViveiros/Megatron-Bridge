...
if is_flash_attn_2_available():
    ...
    raise RuntimeError("Flash Attention 2 is not available, please make sure to install it")
...
class Learnable2DInterpPosEmb(nn.Module):
    ...
    def forward(self, x: torch.Tensor, grid_hws: torch.Tensor) -> torch.Tensor:
        """Add interpolated 2-D positional embeddings to patch tokens.

        Args:
            x:        Packed patch tokens, shape ``[N_total, dim]`` where
                      ``N_total = sum(h_i * w_i)`` across all images in the batch.
            grid_hws: Per-image grid dimensions, shape ``[num_images, 2]``,
                      each row is ``(h_i, w_i)`` in patch units.

        Returns:
            Tensor of shape ``[N_total, dim]`` — same as ``x`` with positional
...
class MoonVitPretrainedModel(PreTrainedModel):
    ...
    _supports_sdpa = True
    # MoonViT has no tied weights. Declared explicitly so newer transformers'
    # from_pretrained (which reads self.all_tied_weights_keys) works with this
    # 4.50-era modeling code.
    _tied_weights_keys = []
