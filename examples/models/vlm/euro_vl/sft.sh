...
# Stage SFT: full joint training — all modules trainable.
#
# Run after completing the PA stage (pa.sh).
# Before training, set WANDB_API_KEY or disable wandb:
# export WANDB_API_KEY=<your_wandb_api_key>
# export WANDB_MODE=disabled

WORKSPACE=${WORKSPACE:-/workspace}
PRETRAINED_CHECKPOINT=${WORKSPACE}/results/euro_vl_2b_pa

echo "Running SFT stage: full joint training (TP=1, PP=1)"
uv run python -m torch.distributed.run --nproc_per_node=1 scripts/training/run_recipe.py \
    ...
    checkpoint.save=${WORKSPACE}/results/euro_vl_2b_sft \
